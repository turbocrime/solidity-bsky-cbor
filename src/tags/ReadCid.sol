...
function _op_cidEq(CidSha256 a, CidSha256 b) pure returns (bool) {
    require(CidSha256.unwrap(a) != 0 && CidSha256.unwrap(b) != 0, "Invalid comparison: zero CID");
...
function isFor(CidSha256 a, bytes memory b) pure returns (bool) {
    require(CidSha256.unwrap(a) != 0, "Invalid check: zero CID");
    require(b.length != 0, "Invalid check: zero content");
...
library ReadCid {
    ...
    function Cid(bytes memory cbor, uint i, bytes1 multicodec) internal pure returns (uint, CidSha256) {
        // require(multicodec == multicodecDagCbor || multicodec == multicodecRaw, "Unsupported multicodec");
        ...
        require(cborMultibase == expect, "Expected CBOR tag 42 and 37-byte CIDv1 of 'raw' or 'DAG-CBOR' type");
    ...
    function NullableCid(bytes memory cbor, uint i) internal pure returns (uint, CidSha256) {
        return cbor.isNull(i) ? (i + 1, CidSha256.wrap(0)) : ReadCid.Cid(cbor, i);
    }
}
